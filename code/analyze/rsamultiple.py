...
def train_model_permutation(X, y, model_type, test_idx, regularization_param=1, max_iter=3000):
    """
    ...
    if model_type == "svm":
        model = LinearSVC(C=regularization_param, max_iter=max_iter)
...
def run(config, sub_i):
    ...
    in_base_dir = base_dir / config_rsa["input_folder"]
    ...
    model_max_iter = config_rsa["model_max_iter"]
    ...
    for epoch_type, epoch_boundary in epoch_type_dict.items():
        ...
        for feat_idx in feat_indices:
            ...
            if feat_idx == -1:  # epoch folder
                in_dir = in_base_dir / "epoch"
            else:
                in_dir = in_base_dir / "cwt"
            bids_in = BIDSPath(
            ...
            if feat_idx == -1:  # epoch folder
                ...
                for cond_i in conds:
                    ...
                    erp.append(condition_amp)
                erp = np.stack(erp, axis=0)  
                features.append(erp)

            else:
                print(f"\n=== Adding frequency band #{feat_idx} ===")
                power_full = np.load(in_file.with_suffix(".npy"))  # Shape: (n_cond, n_trial, n_chan, n_feat, n_time)
                power = power_full[:, :, :, feat_idx, :]
                features.append(power)
        features = np.concatenate(features, axis=2)  # concat across the channel axis
        ...
        for time_idx in time_indices:
            ...
            for cond1 in range(n_cond):
                for cond2 in range(cond1 + 1, n_cond):
                    trials1 = features[cond1, :, :, time_idx]  # Shape: (n_trial, n_chan)
                    trials2 = features[cond2, :, :, time_idx]  # Shape: (n_trial, n_chan)
