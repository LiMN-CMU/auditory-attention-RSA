...
def optimize_hyperparmeters(X, 
    ...
    best_lambda = optimization_result.mean().idxmax()
    # print(optimization_result.mean())
    # print(f"Best lambda: {best_lambda}")
    return best_lambda, optimization_result
...
def run_one_permutation(perm_i, X, y, test_indices, 
    ...
    best_lambda, tuning_table = optimize_hyperparmeters(
        X_train_inner,
        y_train_inner,
    ...
    # Final model on true test set
    y_pred, weights, logit, acc = train_model_permutation(
        X, y, model_type,
        test_idx=test_idx,
        regularization_param=best_lambda
    )
...
def run(config, sub_i):
    ...
    for epoch_type, epoch_boundary in epoch_type_dict.items():
        ...
        _, n_trial, n_chan, n_feat, n_time = power.shape
        ...
        # Loop over features
        all_best_lambdas = np.zeros((len(time_indices), len(cond_pairs), len(test_indices)))
        all_results = []
        for time_i, time_idx in tqdm(enumerate(time_indices)):
            ...
            for cond_pair_i, (cond1, cond2) in enumerate(cond_pairs):
                # trials1 = power[cond1, test_idx, :, feat_idx, :]  # Shape: (n_trial_test, n_chan, n_time)
                # trials2 = power[cond2, test_idx, :, feat_idx, :] 
                # trials1 = power[cond1, :, :, feat_idx, start_time_idx:end_time_idx]  # Shape: (n_trial_test, n_chan, n_time)
                # trials2 = power[cond2, :, :, feat_idx, start_time_idx:end_time_idx] 
                ...
                results = Parallel(n_jobs=-1)(
                    delayed(run_one_permutation)(
                        perm_i, X, y, test_indices,
                        model_type, search_space, n_fold
                    )
                    for perm_i in range(len(test_indices))
                )
                best_lambdas = [r["lambda"] for r in results]
                all_best_lambdas[time_i, cond_pair_i, :] = best_lambdas
                all_results.append(results)
        print(np.unique(all_best_lambdas, return_counts=True))
        
        lambdas, counts = np.unique(all_best_lambdas, return_counts=True)
        plt.figure(figsize=(10, 5))
        plt.bar(lambdas.astype(str), counts)
        plt.xticks(rotation=45)
        plt.xlabel("Lambda (log-spaced)")
        plt.ylabel("Count")
        plt.title("Counts of optimized lambda values")
        plt.tight_layout()
        # plt.savefig("optimize_lambda_hist_total.png")
        plt.savefig(out_file.parent / f"{out_file.stem}_feat-{feat_idx}_model-{model_type}_config-{config_i}_optimize_lambda_hist_total.png")
        plt.close()
        
        # for cond_i in range(len(cond_pairs)):
        #     lambdas_cp = all_best_lambdas[:, cond_i, :].flatten()
        #     lambdas, counts = np.unique(lambdas_cp, return_counts=True)
        ...
        #     plt.xlabel("Lambda (log-spaced)")
        #     plt.ylabel("Count")
        #     plt.title(f"λ distribution - Condition Pair {cond_i}")
        #     plt.tight_layout()
        #     plt.savefig(f"optimize_lambda_hist_pair{cond_i}.png")
